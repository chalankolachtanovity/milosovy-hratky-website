...
STANO = requests.get((os.getenv('t_stano')), auth=('user', 'pass'))
...
def stano_stat():
    ...
    kd_stano = total_kills / total_deaths
    if kd_stano not in stano_kd_list:
        stano_kd_list.append(kd_stano)
    hs_stano = total_kills_headshot * 100 / total_kills
    hs_stano = int(hs_stano)
    if hs_stano not in stano_hs_percentage:
        stano_hs_percentage.append(hs_stano)
    if total_kills_knife not in stano_kills_knife:
        stano_kills_knife.append(total_kills_knife)
    if total_kills_taser not in stano_kills_teaser:
        stano_kills_teaser.append(total_kills_taser)
    if total_kills not in stano_kills:
        stano_kills.append(total_kills)
    if total_deaths not in stano_deaths:
        stano_deaths.append(total_deaths)
    return (f""" 
  <html> 
    <link rel="icon" href="https://e7.pngegg.com/pngimages/117/291/png-clipart-flag-of-hungary-vecteur-flag-miscellaneous-flag-thumbnail.png">
      <body>
        <title>Milaaaaaaaaaaaaaaan</title>
          <h1>Stanove štatistiky</h1>
        {blue_square()}
      </body>
  <body style = "font-family: Verdana, sans-serif;">    
    ...
    <table>  
      ...
      <img src="https://minorityrights.org/wp-content/uploads/2019/04/1996-Flag-of-Hungary.jpg" width="100%" height="20%">
  </html>
  """)
