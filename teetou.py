...
TEETOU = requests.get((os.getenv('t_teetou')), auth=('user', 'pass'))

TEETOU = TEETOU.json()
teetou_kd_list = []
teetou_hs_percentage = []
teetou_kills_taser = []
teetou_kills_knife = []
teetou_kills = []
teetou_deaths = []
...
def teetou_stat():
    teetou = TEETOU['playerstats']['stats']
    for stat in teetou:
    ...
    kd_teetou = total_kills / total_deaths
    if kd_teetou not in teetou_kd_list:
        teetou_kd_list.append(kd_teetou)
    hs_teetou = total_kills_headshot * 100 / total_kills
    hs_teetou = int(hs_teetou)
    if hs_teetou not in teetou_hs_percentage:
        teetou_hs_percentage.append(hs_teetou)
    if total_kills_knife not in teetou_kills_knife:
        teetou_kills_knife.append(total_kills_knife)
    if total_kills not in teetou_kills:
        teetou_kills.append(total_kills)
    if total_deaths not in teetou_deaths:
        teetou_deaths.append(total_deaths)
    return (f"""
  <html>
      <link rel="icon" href="https://i1.sndcdn.com/artworks-000157051671-azmuka-t500x500.jpg">
      <body>
        <title>Rííííííííížo</title>
          <h1>Rížové štatistiky</h1>
        {blue_square()}
      </body>
  <body style = "font-family: Verdana, sans-serif;">    
    ...
    <table>
      <h3>Rifles</h3>
        ...
        <tr><td>Total kills LAMA CT:</td><td>| {total_kills_scar20} |--</td><td>|VS|-- Total kills AUTO T:</td><td>| {total_kills_g3sg1} |</td>
    </table>
    {blue_square()}
    <table>
      <h3>SMG's</h3>
    ...
    <table>  
      ...
      <img src="https://www.kfcslovakia.sk/wp-content/uploads/2018/12/kfc_buckety_hw30-400x400.jpg" alt="HTML5 Icon" width="100%" height="30%">
  </html>  
  </html>
  """)
